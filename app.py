...
@app.get("/api/dailies")
def list_dailies(admin=Depends(require_admin)):
    with conn() as c:
        rows = c.execute("SELECT payload FROM dailies ORDER BY daily_date DESC, saved_at DESC").fetchall()
    return [json.loads(r["payload"]) for r in rows]

@app.get("/api/dailies/{daily_id}/verify")
def verify_daily(daily_id: str, user=Depends(current_user)):
    with conn() as c:
        row = c.execute("SELECT payload FROM dailies WHERE id=?", (daily_id,)).fetchone()
    if not row:
        raise HTTPException(404, "daily not found")
    rec = json.loads(row["payload"])
    if user["role"] != "admin" and rec.get("createdBy") != user["username"] and rec.get("updatedBy") != user["username"]:
        raise HTTPException(403, "not allowed")
    return {"ok": True, "id": daily_id}
...
def upsert_daily(daily_id: str, body: Daily, user=Depends(current_user)):
    ...
    with conn() as c:
        existing = c.execute("SELECT payload FROM dailies WHERE id=?", (daily_id,)).fetchone()
        if existing and user["role"] != "admin":
            old_rec = json.loads(existing["payload"])
            if old_rec.get("createdBy") != user["username"]:
                raise HTTPException(403, "not allowed")
